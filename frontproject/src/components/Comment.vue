  ...
  export default {
    created() {
      ...
      axios({
        ...
        console.log("现在输出评论区数据")
        console.log(res);
        for(let i=0; i<res.data.SonComments.length; i++){
          for(let j=0; j<res.data.SonComments[i].length; j++){
            let obj = {}
            obj.ccid = res.data.SonComments[i][j].pk
            obj.cid = res.data.SonComments[i][j].fields.comment
            obj.userId = res.data.SonComments[i][j].fields.uid
            obj.time = this.TimeFormat(res.data.SonComments[i][j].fields.commentTime)
            obj.content = res.data.SonComments[i][j].fields.content
            this.soncomments.push(obj)
          }
        }
        for(let a of res.data.commentList){
          let obj = {}
          obj.cid = a.pk
          obj.userId=a.fields.uid
          obj.time = this.TimeFormat(a.fields.commentTime)
          obj.content = a.fields.content
          this.comments.push(obj)
        };
        for(let i=0; i<res.data.commenter.length; i++){
          this.comments[i].userName = res.data.commenter[i]
        };
        let i=0;
        for(let j=0; j<res.data.soncommenter.length; j++){
          for(let k=0; k<res.data.soncommenter[j].length; k++){
            this.soncomments[i++].userName = res.data.soncommenter[j][k];
          }
        }
        console.log("这里是评论区结尾")
      })
    },
    name: "Comment",
    data() {
