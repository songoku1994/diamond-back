<template>
  <div>
    <el-dialog title="新建文档" v-if="visible" :visible.sync="visible" width="490px" top="5vh" :show-close="false">
      ...
      <div>所属团队:</div><br>
      <div>
        <el-select v-model="NewFile.TeamId" disabled>
          <el-option :label="NewFile.TeamName" :value="NewFile.TeamId">{{NewFile.TeamName}}</el-option>
        </el-select>
      </div>
      <br>
      <div>
        ...
        <div>
          <el-select v-model="NewFile.Authority">
            <el-option v-for="(i,index) in Authority" v-if="NewFile.TeamId!==-1||(NewFile.TeamId===-1&&(index<1||index>2))" :label="i" :value="index">{{i}}</el-option>
          </el-select>
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        NewFile:{Title:null,SimpleMessage:null,TeamId:null,Authority:0,Revise:0,TeamName:''},
        Authority:['仅自己','团队成员可见','团队成员可编辑','所有人可见','所有人可编辑'],
      }
    },
    created() {
      //从TeamManage中传入的团队名字和团队ID
      this.NewFile.TeamId=this.TeamId
      this.NewFile.TeamName=this.TeamName
    },
    methods:{
      submit(){
    ...
    watch:{
      'NewFile.TeamId'(){
        if(this.NewFile.TeamId===-1&&1<=this.NewFile.Authority&&this.NewFile.Authority<=2){
          this.NewFile.Authority=0;
        }
      },
      'NewFile.Authority'(){
